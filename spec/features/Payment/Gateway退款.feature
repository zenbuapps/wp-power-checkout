...
Feature: Gateway 退款
  ...
  Rule: 前置（狀態）- Gateway 必須是 AbstractPaymentGateway 實例
  ...
  Rule: 後置（狀態）- 退款成功時建立退款記錄並呼叫 SLP API
    Example: 信用卡全額退款成功
      ...
      And SLP 退款 API 回傳成功（refundMsg.code 為 null）
      When 用戶發送 POST /wp-json/power-checkout/v1/refund
        | key      | value |
        | order_id | 100   |
      Then 回應狀態碼為 200
      ...
      And 訂單 #100 的 tmp_refund_reason 已儲存

  Rule: 後置（狀態）- 退款 API 失敗時回滾
    Example: SLP 退款 API 回傳失敗
      Given 用戶 "Admin" 已登入並取得 Nonce
      And 訂單 #100 付款方式為 CreditCard
      And SLP 退款 API 回傳失敗（refundMsg.code 有值）
      When Gateway handle_payment_gateway_refund 被觸發
      Then 資料庫事務被 ROLLBACK
      And 退款記錄被刪除
      And 訂單 #100 有 order note 包含 "退款失敗"

  Rule: 後置（狀態）- 手動退款（非 API 退款）不觸發 Gateway 退款邏輯
    Example: 手動建立的退款記錄不走 Gateway
      Given 訂單 #100 有一筆手動退款（get_refunded_payment 為 false）
      When woocommerce_order_refunded hook 被觸發
      Then 不呼叫 SLP 退款 API
