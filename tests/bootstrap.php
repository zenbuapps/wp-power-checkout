 ...
 * 順序：WooCommerce → Powerhouse → Power Checkout
 */
function _power_checkout_integration_manually_load_plugin(): void {
	// 1. 載入 WooCommerce
	...
	// 3. 載入 Power Checkout plugin
	require dirname( __DIR__ ) . '/plugin.php';
}

tests_add_filter( 'muplugins_loaded', '_power_checkout_integration_manually_load_plugin' );
 ...
 */
function _power_checkout_integration_force_bootstrap(): void {
	if ( class_exists( 'J7\PowerCheckout\Bootstrap' ) ) {
		\J7\PowerCheckout\Bootstrap::instance();
	}
}

tests_add_filter( 'plugins_loaded', '_power_checkout_integration_force_bootstrap', 20 );
